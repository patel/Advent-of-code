...
def decode_diff(input_str):
    diff = 0
    for line in input_str.split('\n'):
        diff = 2 + diff + len(re.findall(r'(\\x[a-fA-F0-9]{2})', line[1:-1])) * 3 + \
               len(re.findall(r'(\\\\)', line[1:-1])) * 1 + \
               len(re.findall(r'(\\")', line[1:-1])) * 1
    return diff


def encode_diff(input_str):
    diff = 0
    for line in input_str.split('\n'):
        diff = 2 + diff + \
...
# Part 1
print decode_diff(input_str)

# Part 2
print encode_diff(input_str)
