class Solution(object):
	def calcNum(self, input):
		total_nums = 0
		for line in input.splitlines():
			signal_patterns, four_digits = line.split(' | ')
			total_nums += len(filter(lambda x: len(x) in [2,3,4,7], four_digits.split(' ')))
		return total_nums

	def decodeSignal(self, new_map):
		default_lookup = {
		...
		five_lookup = {}
		for element in new_map[5]:
			for e in element:
				if e not in five_lookup:
					five_lookup[e] = 0
				five_lookup[e] += 1
		five_chars_by_size = {}
		for k,v in five_lookup.items():
			if v not in five_chars_by_size:
				five_chars_by_size[v] = []
			five_chars_by_size[v].append(k)

		real_signal_lookup = {}
		real_signal_lookup[frozenset('a')] = (new_map[3][0] - new_map[2][0])
		real_signal_lookup[frozenset('f')] = (new_map[3][0] & new_map[2][0] & new_map[4][0] & new_map[6][0] & new_map[6][1] & new_map[6][2] & new_map[7][0])
		real_signal_lookup[frozenset('c')] = (new_map[2][0] - real_signal_lookup[frozenset('f')])
		real_signal_lookup[frozenset({'d','g'})] = (new_map[5][0] & new_map[5][1] & new_map[5][2]) - (real_signal_lookup[frozenset('a')]) 
		real_signal_lookup[frozenset({'b','e'})] = frozenset(five_chars_by_size[1])
		real_signal_lookup[frozenset('b')] = real_signal_lookup[frozenset({'b','e'})] & new_map[4][0]
		real_signal_lookup[frozenset('d')] = new_map[4][0] -  real_signal_lookup[frozenset('b')] - real_signal_lookup[frozenset('c')] - real_signal_lookup[frozenset('f')]
		real_signal_lookup[frozenset('g')] = real_signal_lookup[frozenset({'d','g'})] - real_signal_lookup[frozenset('d')]
		real_signal_lookup[frozenset('e')] = real_signal_lookup[frozenset({'b','e'})] - real_signal_lookup[frozenset('b')]

		new_num_lookup = {}
		for signal,v in default_lookup.items():
			new_digit_signal = frozenset()
			for el in signal:
				new_digit_signal = new_digit_signal.union(real_signal_lookup[frozenset(el)])
			new_num_lookup[new_digit_signal] = v
		return new_num_lookup

	def calcNum2(self, input):
		total_nums = 0
		for line in input.splitlines():
			signal_patterns, four_digits = line.split(' | ')
			signal_lookup_by_len = {}
			for signal_pattern in signal_patterns.split(' '):
				if len(signal_pattern) not in signal_lookup_by_len:
					signal_lookup_by_len[len(signal_pattern)] = []
				signal_lookup_by_len[len(signal_pattern)].append(frozenset(list(signal_pattern)))
			new_signal_to_digits = self.decodeSignal(signal_lookup_by_len)
			new_num = 0
			for digit in four_digits.split(' '):
				new_num = new_num*10+new_signal_to_digits[frozenset(list(digit))]
			total_nums += new_num	
		return total_nums


s = Solution()
print(s.calcNum2('''dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
