...
def parse_input(input_str):
    ...
    for line in input_str.split('\n'):
        matching_groups = re.match(r'(.*) ([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)', line)
        instructions.append((matching_groups.group(1), int(matching_groups.group(2)), int(matching_groups.group(3)),
                             int(matching_groups.group(4)), int(matching_groups.group(5))))
    return instructions


def process_instructions_1(instructions, grid):
    for instruction in instructions:
        command, init_row, init_column, final_row, final_column = instruction
        for pair in [(r, c) for r in range(init_row, final_row + 1) for c in range(init_column, final_column + 1)]:
            r, c = pair
            if command == 'turn on':
                grid[r][c] = True
...
def process_instructions_2(instructions, grid):
    for instruction in instructions:
        command, init_row, init_column, final_row, final_column = instruction
        for pair in [(r, c) for r in range(init_row, final_row + 1) for c in range(init_column, final_column + 1)]:
            r, c = pair
            if command == 'turn on':
                ...
                grid[r][c] = grid[r][c] - 1 if grid[r][c] > 1 else 0
            elif command == 'toggle':
                grid[r][c] = grid[r][c] + 2


input_str = '''turn on 887,9 through 959,629
...
grid_1 = [x[:] for x in [[False] * 1000] * 1000]
instructions = parse_input(input_str)

# Part 1
process_instructions_1(instructions, grid_1)
print sum(map(lambda x: sum(filter(lambda y: y, x)), grid_1))

grid_2 = [x[:] for x in [[0] * 1000] * 1000]

# Part 2
process_instructions_2(instructions, grid_2)
print sum(map(lambda x: sum(filter(lambda y: y, x)), grid_2))
