...
def parse_input_into_dict(input_str):
    sues_lookup = defaultdict(dict)
    for line in input_str.split('\n'):
        matches = re.match(r'Sue ([0-9]*): (.*)', line)
        sues_lookup[matches.group(1)] = {v[0]: v[1] for v in map(lambda a: a.split(': '), matches.group(2).split(', '))}
    return sues_lookup


def find_index_with_matching_option(all_sues, MFCAM_op):
...
all_sues = \
    parse_input_into_dict('''Sue 1: goldfish: 9, cars: 0, samoyeds: 9
