...
def calcCaliberationValues(input, is_part_two = False):
	total_caliberation = 0
	lookup = {'one':'one1one', 'two':'two2two', 'three':'three3three', 'four': 'four4four', 'five':'five5five', 'six':'six6six', 'seven':'seven7seven', 'eight':'eight8eight', 'nine':'nine9nine'}
	for line in input.split('\n'):
		if is_part_two:
			for key in lookup.keys():
				line = re.sub(key, lookup[key], line)
		arr = [int(x) for x in list(line) if x.isnumeric()]
		caliberation = arr[0]*10 + arr[-1]
		total_caliberation += caliberation
	return total_caliberation


print(calcCaliberationValues('''two934seven1
8825eightknfv
sevenoneqbfzntsix55
...
sixgtxr2fourrdkjg
fivebxsevensixone872dlx''', is_part_two = True))
