...
def getMatchingGames(input, goal = {'blue':10, 'red':10, 'green':10}):
	s = 0
	for line in input.split('\n'):
		game_identifier, sets = line.split(': ')
		_, game_num = game_identifier.split(' ')
		s += int(game_num)
		should_skip_game = False
		for s_set in sets.split('; '):
			if should_skip_game:
				break
			set_params = s_set.split(', ')
			param_pairs = [s.split(' ') for s in set_params]
			for num, param in param_pairs:
				if int(num) > goal[param]:
...
def getMatchingGamesPartTwo(input):
	s = 0
	for line in input.split('\n'):
		...
		for s_set in sets.split('; '):
			...
			param_pairs = [s.split(' ') for s in set_params]
			for num, param in param_pairs:
				if int(num) > min_cubes[param]:
					min_cubes[param] = int(num)
		power = min_cubes['blue']*min_cubes['green']*min_cubes['red']
		s += power
	return s


print(getMatchingGames(input, goal={'red':12, 'green':13, 'blue': 14}))
print(getMatchingGamesPartTwo(input))
