...
input = '''.......12.......935............184.720...243........589.652..........435..........483.............6...........................904...........
...
def processInput(input):
	...
	for line in input.split('\n'):
		...
		for i in range(len(list_line)):
			d[line_num][i] = list_line[i]
		curr_num = 0
		curr_num_indices = []
		for i,x in d[line_num].items():
			if x.isnumeric():
				curr_num = curr_num*10 + int(x)
				curr_num_indices.append((line_num, i))
			else:
				if curr_num:
					for coordinate in curr_num_indices:
						co_to_nums[coordinate] = curr_num
					num_to_cos.append((curr_num, curr_num_indices))
					co_to_nums[curr_num] = curr_num_indices
					curr_num_indices = []
					curr_num = 0
		if curr_num:
			for coordinate in curr_num_indices:
			...
			curr_num = 0
		line_num += 1
	return (d, num_to_cos, co_to_nums) 

def calSumPartsSchematics(input, is_part_two=False):
	remaining = []
	sum_parts = 0
	d, num_to_cos, co_to_nums = processInput(input)
	if is_part_two:
		gear_ratios = 0
		for i, l in d.items():
			for j, v in l.items():
				if v == '*':
					...
					for (x,y) in [(i+1, j),(i, j+1), (i-1, j),(i, j-1),(i+1, j-1),(i+1, j+1), (i-1, j-1), (i-1, j+1)]:
						if (x,y) in co_to_nums:
							gear_nums.add(co_to_nums[(x,y)])
					if len(gear_nums) == 2:
						gear_ratios += (gear_nums.pop()*gear_nums.pop())

		return gear_ratios

	for number, coordinates in num_to_cos:
		skip_num = False
		for (x,y) in coordinates:
			if d[x-1][y] in SYMBOLS or d[x+1][y] in SYMBOLS or d[x-1][y-1] in SYMBOLS or d[x-1][y+1] in SYMBOLS or d[x][y-1] in SYMBOLS or d[x][y+1] in SYMBOLS or d[x+1][y-1] in SYMBOLS or d[x+1][y+1] in SYMBOLS:
				sum_parts += number
				skip_num = True
				break
		if not skip_num:
...
print(calSumPartsSchematics(input))
print(calSumPartsSchematics(input, is_part_two=True))
